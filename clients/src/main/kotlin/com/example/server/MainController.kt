...
import com.example.flow.ReinsuranceRenewalFlow.Initiator
...
class MainController(rpc: NodeRPCConnection) {
    ...
    fun getIOUs() : ResponseEntity<List<StateAndRef<ReinsuranceRenewalState>>> {
        return ResponseEntity.ok(proxy.vaultQueryBy<ReinsuranceRenewalState>().states)
    }
    ...
    fun getMyIOUs(): ResponseEntity<List<StateAndRef<ReinsuranceRenewalState>>>  {
        val myious = proxy.vaultQueryBy<ReinsuranceRenewalState>().states.filter { it.state.data.lender.equals(proxy.nodeInfo().legalIdentities.first()) }
